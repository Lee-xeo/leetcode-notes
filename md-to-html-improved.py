...
"""
将Markdown文件转换为HTML，同时合并相邻的Python和C++代码块
使用说明：
...
class CodeBlockProcessor:
    ...
    def process_code_blocks(self, md_content):
        """
        处理Markdown中的代码块，合并相邻的Python和C++代码块
        返回替换了代码块的Markdown内容
        """
        # 正则表达式匹配代码块: ```语言 代码 ```
        code_block_pattern = r'```(python|cpp)(.*?)```'
        ...
        for i in reversed(range(len(matches))):
            if i in processed_blocks:
                ...
                # 如果是对的第二个，则删除
                # 如果是单独的代码块，则替换为代码容器
                if i > 0 and i - 1 in processed_blocks and matches[i-1].group(1) == 'python' and match.group(1) == 'cpp':
...
def convert_markdown_to_html(md_content):
    """转换Markdown为HTML，处理代码块"""
    # 处理代码块
    code_processor = CodeBlockProcessor()
    processed_md = code_processor.process_code_blocks(md_content)
    ...
    def replace_placeholder(match):
        ...
        return code_processor.get_code_container(index)
...
def generate_toc(html_content):
    ...
    toc_html = '<div id="sidebar-content">\n<ul id="toc">\n'
